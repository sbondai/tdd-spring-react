...
describe("UserPage", () => {
  describe("Layout", () => {
    ...
    it("displays the displyName@username when user data loaded", async () => {
    ...
    it("displays not found when user not found", async () => {
  ...
  describe("ProfileCard Interactions", () => {
    ...
    it("displays the edit layout when Clicking edit button", async () => {
      setUserOneLoggedInStorage();
    ...
    xit("displays spinner when there is updateUser api call", async () => {
    ...
    xit("removes validation error if user cancels", async () => {
      const { queryByText, queryByRole } = await setupForEdit();
      apiCalls.updateUser = jest.fn().mockRejectedValue(mockFailUpdateUser);
